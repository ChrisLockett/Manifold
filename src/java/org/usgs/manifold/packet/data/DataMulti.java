...
public class DataMulti extends DataPacket{
    ...
    /**
     * @return the collected samples after adding the offset value
     */
    public int[] getDataWithOffset(Integer offset) {
    	int [] datawithoffset = new int[samples];
    	for(int i = 0; i < samples; ++i ){
    		datawithoffset[i] = data[i] + offset;
    	}
    	return datawithoffset;
    }    
    
    /**
     * removes last sample in the Data
     */ 
    public void dropLastSample() {
        //System.out.println("PRE  :starttime = " +startTime+ " endtime = "+endTime+ " samplerate "+ sampleRate +" samples "+samples);
        --samples;
        sampleRate = (float)samples;
        int [] newdata = new int[samples];
        for(int i = 0; i < samples; ++i ){
    		newdata[i] = data[i];
    	}
        data = newdata;
        //System.out.println("POST : starttime = " +startTime+ " endtime = "+endTime+ " samplerate "+ sampleRate +" samples "+samples);
    }
    
    /**
     * adds an extra samples at the end of data same value as the value be for it
     */ 
    public void addExtraSample() {
        ++samples;
        sampleRate = (float)samples;
        int [] newdata = new int[samples];
        ...
        newdata[samples-1] = data[samples-2];
        data = newdata;
    }
